...
**PRD-208-R3.** *(Core)* By default, the factory MUST validate every emitted node against `schemas/100/node.schema.json` AND every content block on that node against the applicable PRD-102 block-level schema for the block's `type`, before passing the node to the framework. Validation runs inside the factory's `transform` wrapper, after the user-supplied `transform` returns and before the framework receives the node:

- For full-node emissions (per PRD-200-R5):
  1. Validate the node envelope against `schemas/100/node.schema.json`.
  2. For each entry in the node's `content` array, validate the block against the applicable PRD-102 block schema:
     - `type: "markdown"` → `schemas/102/block-markdown.schema.json` (PRD-102-R1).
     - `type: "prose"` → `schemas/102/block-prose.schema.json` (PRD-102-R2).
     - `type: "code"` → `schemas/102/block-code.schema.json` (PRD-102-R3).
     - `type: "data"` → `schemas/102/block-data.schema.json` (PRD-102-R4).
     - `type: "callout"` → `schemas/102/block-callout.schema.json` (PRD-102-R5).
     - `type: "marketing:*"` → `schemas/102/block-marketing-namespace.schema.json` (PRD-102-R6).
     - Any other `type` value: validation MUST fail with the same closed-set error path as the schema-validator's "unknown block type" finding (PRD-102's discriminator is exhaustive at v0.1).
- For partial emissions (per PRD-200-R5 with `_actPartial: true`), validate that `id` is present and conforms to PRD-100-R10; if a partial supplies a `content` array, each block in it MUST also pass its applicable PRD-102 block schema. Other PRD-100-R21 envelope fields are deferred to the merge step (PRD-200-R12).
- For `null` (deliberate skip per PRD-200-R5), no validation — the framework records the skip.

Validation failure (envelope OR block-level) produces an unrecoverable error per PRD-208-R12, with the error message citing the node's `id` (when present), the offending block's index in the `content` array (when block-level), and the specific JSON-Schema violation. When `validate: "off"` is set, the factory MUST skip validation (envelope and block) but MUST emit a build warning at `init` citing the operator's opt-out and the resulting risk surface.

Rationale: PRD-704's e-commerce catalog example (and similar adapters that synthesize structured `data` blocks) depends on the factory catching block-level shape violations pre-emit so that downstream consumers never receive a node whose `data` block fails its taxonomy schema. Validating the envelope alone (which treats `content` as an open array of objects) leaves block-level malformations to surface at PRD-600 conformance time, well after the build has emitted to disk. PRD-208 is the canonical opt-in point for pre-emit safety; extending validation to blocks is faithful to PRD-208-R3's stated intent.
...
- `fixtures/208/negative/transform-emits-malformed-data-block.expected.json` → R3, R12. User returns a node whose `content` array carries a `data` block missing the required `data` payload (PRD-102-R4); factory's pre-emit block-level validation rejects with an unrecoverable error citing the block index. Cites amendment A3 (closed 2026-05-02).
...
| 2026-05-02 | Spec Steward | Inline clarification per amendment A3 (SOP-3): expanded PRD-208-R3 to require pre-emit validation of (a) the node envelope against `schemas/100/node.schema.json` AND (b) every entry in `content` against the applicable PRD-102 block-level schema (markdown / prose / code / data / callout / marketing:*). Added per-block-type schema map and `block_index` to the error message. Closes the gap that let malformed `data` / `marketing:*` blocks slip past the factory and only surface at PRD-600 conformance time (per PRD-704 e-commerce catalog example). New negative fixture `fixtures/208/negative/transform-emits-malformed-data-block.expected.json`. PRD remains Accepted; default `validate: "before-emit"` semantics are tightened (existing user code that returns well-formed blocks is unaffected; user code that returned malformed blocks was already non-conformant per PRD-102). |
