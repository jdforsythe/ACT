...
- <!-- proposed amendment A2 --> `parseMode` (string, OPTIONAL; one of `"coarse"`, `"fine"`; default `"coarse"`) — pass-through to PRD-201's `mode` config (PRD-201-R12). When set, the plugin MUST forward the value to every auto-wired PRD-201 adapter instance (docs / blog / static-`.md` pages). When omitted, the plugin preserves the PRD-201 default (`"coarse"`). The `parseMode` setting is independent of the React-component extraction mode (`extractMode`, PRD-404-R7) and the conformance target (`target`, PRD-404-R14). Setting `parseMode: "fine"` against a `target: "core"` build MUST fail at `init` per PRD-201-R23's level-mismatch rule (the markdown adapter refuses fine-mode emission against a Core target); the plugin surfaces the underlying adapter error verbatim. Conformance: **Core** (the field; **Standard** for the fine-grained behavior the field unlocks per PRD-201-R12).
...
export interface ActDocusaurusOptions extends Partial<Omit<GeneratorConfig, "outputDir" | "baseUrl">> {
   ...
   * "fine" against `target: "core"` fails at init per PRD-201-R23.
   * Added per amendment A2 (PRD-404 In review, MINOR bump per PRD-108-R4(1)).
   */
...
| 2026-05-02 | Spec Steward | **MINOR bump per PRD-108-R4(1)** (additive optional field). Status: Accepted → In review for amendment A2 (SOP-4): added `parseMode` to PRD-404's `GeneratorConfig` shape (PRD-404-R16) and to the TypeScript `ActDocusaurusOptions` interface as a pass-through to PRD-201's `mode` config (PRD-201-R12). Default `"coarse"` preserves the existing v0.1 behavior; `"fine"` is the opt-in. Cross-cutting recommendation: **sibling generators PRD-402 (Hugo, spec-only), PRD-403 (MkDocs, spec-only), and PRD-408 (Eleventy) should receive the same additive `parseMode` field for parity** — without it, fine-grained-mode-via-generators is unreachable for those hosts and PRD-701-style large-docs deployments must bypass the generator framework. Suggest the BDFL sweep this in a single coordinated MINOR bump across 402 / 403 / 404 / 408 to avoid an asymmetric ecosystem. PRD-404 awaits BDFL sign-off; on approval, status flips back to Accepted with a fresh Changelog row. |
| 2026-05-02 | Jeremy Forsythe (BDFL) | Status: In review → Accepted. BDFL sign-off on amendment A2 (additive `parseMode` field; default `"coarse"` is non-breaking; MINOR bump per PRD-108-R4(1)). Sibling-sweep recommendation accepted for Phase 6.2 follow-up: PRD-408 (Eleventy) takes the same `parseMode` field at the start of Track B (Adapter/Generator Engineer owns); PRD-402 (Hugo, spec-only) and PRD-403 (MkDocs, spec-only) get a coordinated MINOR bump in a single sweep before v0.1 ship. New amendment IDs A10 (PRD-408), A11 (PRD-402), A12 (PRD-403) filed for tracking. |
