...
[params.act]
  ...
  incremental = true
  parseMode = "coarse"                   # "coarse" | "fine"; default "coarse"; PRD-201-R12 (A11)
...
The `parseMode` key (string, OPTIONAL; one of `"coarse"`, `"fine"`; default `"coarse"`) is the canonical pass-through to PRD-201's `mode` config (PRD-201-R12), added per amendment A11 (sibling sweep of A2 / A10 — keeps the parse-mode reach symmetric across hosts so community Hugo ports can expose the same knob as the TS-first-party generators). When set, the module MUST forward the value to the content-walk's body-to-block mapping (PRD-402-R12). When omitted, the module preserves the PRD-201 default (`"coarse"`). The `parseMode` setting is independent of `conformanceTarget`. Setting `parseMode = "fine"` against `conformanceTarget = "core"` MUST fail at module init per PRD-201-R23's level-mismatch rule (the body-to-block contract refuses fine-mode emission against a Core target); the module surfaces the underlying error verbatim. The legacy `mode` key (introduced in PRD-402-R12's body-to-block mapping) is accepted as an alias of `parseMode` for backward compatibility; when both are supplied with conflicting values, the module MUST surface a configuration error. Conformance: **Core** (the field; **Standard** for the fine-grained behavior the field unlocks per PRD-201-R12). MINOR bump per PRD-108-R4(1).
...
| 2026-05-02 | Spec Steward | **MINOR bump per PRD-108-R4(1)** (additive optional field). Inline edit per SOP-3 of amendment A11 (sibling sweep of A2 / A10; coordinated pre-ship sweep accepted by BDFL on 2026-05-02). Added `parseMode` (`"coarse" \| "fine"`, default `"coarse"`) to PRD-402-R4's `[params.act]` configuration shape as the canonical pass-through to PRD-201's `mode` config (PRD-201-R12). The legacy `[params.act].mode` key (introduced in PRD-402-R12's body-to-block mapping prose) is preserved as an alias of `parseMode`; conflicting values surface a configuration error. Default `"coarse"` preserves byte-identical behavior for every pre-amendment Hugo deployment; `"fine"` is opt-in. The level-mismatch rule from PRD-201-R23 applies — `parseMode = "fine"` against `conformanceTarget = "core"` fails at module init. PRD-402 stays Accepted (spec-only per decision Q3; the field is for community Go ports + future first-party impls). |
