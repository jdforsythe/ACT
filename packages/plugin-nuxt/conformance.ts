/**
 * PRD-407 conformance gate (PRD-400-R28 + PRD-407-R22): builds a tiny
 * Nuxt-shaped fixture site programmatically (no `nuxt` CLI) using the
 * same `runActBuild` entry point the Nuxt module's `build:done` hook
 * invokes, then validates the emitted files via `@act-spec/validator`.
 * Exits non-zero on any gap.
 *
 * Why programmatic? Spinning a real `nuxt generate` in CI requires Nuxt
 * + Vite + a working SFC toolchain; the integration's pipeline is
 * identical whether invoked from Nuxt's `build:done` or from
 * `runActBuild`.
 *
 * Invoked by `pnpm -F @act-spec/plugin-nuxt conformance`.
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const cfg: GeneratorConfig = {
      ...
      site: { name: 'Acme Vue' },
      generator: '@act-spec/plugin-nuxt@0.0.0',
    };
