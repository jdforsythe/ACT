 ...
 *     from `@act-spec/markdown-adapter`. No adapter logic is duplicated
 ...
 *     the `@act-spec/docusaurus` and `@act-spec/nuxt` posture).
 */
...
const PACKAGE_NAME = '@act-spec/eleventy' as const;
...
export function enforceEleventyVersion(eleventyConfig: EleventyConfigLike): void {
  if (typeof eleventyConfig.versionCheck !== 'function') {
    throw new Error(
      `PRD-408-R2: @act-spec/eleventy requires Eleventy ${ELEVENTY_PEER_FLOOR}.0+. ` +
...
export function validateOptions(options: unknown): EleventyActOptions {
  ...
  if ('bindings' in opts) {
    throw new Error(
      'PRD-408-R10: @act-spec/eleventy does not support component bindings. ' +
        'Eleventy is template-driven; component instrumentation is out of scope. ' +
        'For component-driven workflows, see @act-spec/astro (PRD-401), ' +
        '@act-spec/nextjs-static (PRD-405), or @act-spec/nuxt (PRD-407).',
    );
  }
 ...
 * (matching the @act-spec/docusaurus / A2 pattern).
 *
...
export function resolveConfig(args: {
  ...
  // constructed. Mirrors @act-spec/docusaurus's A2 pattern: the error text
 ...
 * `@act-spec/docusaurus`'s `runActBuild`: takes a fully-resolved
...
export function actEleventyPlugin(
  ...
  // (mirrors @act-spec/docusaurus's pre-flight pattern).
