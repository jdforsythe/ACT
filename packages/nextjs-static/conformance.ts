/**
 * PRD-405 conformance gate (PRD-400-R28 + PRD-405-R21): builds a tiny Next-
 * shaped fixture site programmatically (no `next` CLI) using the same
 * `runActBuild` entry point Next's webpack post-build plugin invokes,
 * then validates the emitted files via `@act-spec/validator`. Exits
 * non-zero on any gap.
 *
 * Why programmatic? Spinning a real `next build` in CI requires `next` and
 * a JSX toolchain; the integration's pipeline is identical whether invoked
 * from `compilation.hooks.done` or from `runActBuild`.
 *
 * Invoked by `pnpm -F @act-spec/nextjs-static conformance`.
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const cfg: GeneratorConfig = {
      ...
      site: { name: 'Acme' },
      generator: '@act-spec/nextjs-static@0.0.0',
    };
