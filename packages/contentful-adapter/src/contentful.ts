/**
 * PRD-202 Contentful adapter — `@act-spec/contentful-adapter`.
 *
 ...
 *    `@act-spec/validator` and `@act-spec/programmatic-adapter`.
...
function findPackageRoot(start: string): string {
  ...
  for (let i = 0; i < 6; i += 1) {
    try {
      ...
      if (pkg.name === '@act-spec/contentful-adapter') return dir;
    } catch {
      // keep climbing
    }
